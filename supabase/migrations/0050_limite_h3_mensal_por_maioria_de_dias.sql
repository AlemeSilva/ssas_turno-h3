-- =====================================================================
-- O limite_h3_mensal contava H3 pelo mês do PRÓPRIO semana_ref (o
-- sábado em que a semana começa), não pelo mês onde a maioria dos 7
-- dias da semana realmente cai. Numa semana de fronteira isso atribui
-- o turno inteiro ao mês errado.
--
-- Achado real, 2026-09-11: a semana de 31/10 a 06/11/2026 (semana_ref
-- 2026-10-31) tem só 1 dia em outubro e 6 em novembro — mas contava
-- como o H3 de OUTUBRO do Caique (limite_h3_mensal=1), bloqueando-o de
-- fazer um H3 genuinamente de outubro (ex.: 17-23/10) mesmo sem ter
-- feito nenhum turno completo nesse mês. Pedido do Gerente: uma semana
-- só deve "gastar" o limite do mês onde efetivamente trabalha a maior
-- parte dos dias — até 3 dias de piquete num mês não deve contar como
-- um turno completo desse mês.
--
-- Como 7 é ímpar, o 4º dia da semana (semana_ref + 3) cai sempre no
-- mês com maioria de dias (nunca há empate 3-4 possível ao contrário —
-- só existe 0/7, 1/6, 2/5 ou 3/4 de split, e o dia do meio segue sempre
-- o lado maior). Por isso date_trunc('month', semana_ref + 3) dá
-- diretamente "o mês a que esta semana pertence", sem precisar de
-- calcular os dois lados do split.
--
-- Verificado com os dados reais do Caique (ago-nov/2026) antes desta
-- migração: setembro mantém-se com 2 H3 (histórico anterior a esta
-- regra, não corrigido retroativamente — mesmo critério já usado nas
-- migrações 0047/0048 para não reabrir o passado); outubro passa a ter
-- só a semana de 17/10 (a de 31/10 passa a contar para novembro).
--
-- Não corrige dados já gravados — só passa a interpretar corretamente
-- o que já existe, e aplica-se a partir de agora em qualquer inserção
-- ou atualização de H3, tanto manual como no preenchimento automático
-- anual (preencher_escala_anual, abaixo).
-- =====================================================================
    ...
    if new.turno = 'H3' then
        ...
        if v_limite is not null then
            ...
            where usuario_id = new.usuario_id
              ...
              and date_trunc('month', semana_ref + 3) = date_trunc('month', new.semana_ref + 3);
            if v_count >= v_limite then
                raise exception 'Utilizador já atingiu o limite de % H3/mês em %.', v_limite,
                    to_char(new.semana_ref + 3, 'MM/YYYY');
            end if;
        end if;
    end if;
    return new;
...
begin
    ...
    loop
        begin
            ...
            v_mes_alvo := date_trunc('month', v_semana + 3)::date;

            select array_agg(id order by cnt_3m asc, cnt_ano asc, id asc) into v_ordenados
              from (
                  select cand.id,
                         (select count(*) from escala_semanal e
                           where e.turno = 'H3' and e.usuario_id = cand.id
                             and e.semana_ref >= v_tres_meses_atras and e.semana_ref < v_semana) as cnt_3m,
                         (select count(*) from escala_semanal e
                           where e.turno = 'H3' and e.usuario_id = cand.id
                             and e.semana_ref >= v_inicio_ano and e.semana_ref < v_semana) as cnt_ano
            ...
            foreach v_cand in array v_ordenados loop
                ...
                select count(*) into v_contagem_mes
                 ...
                 where turno = 'H3' and usuario_id = v_cand
                   and date_trunc('month', semana_ref + 3) = v_mes_alvo
                   and semana_ref < v_semana;
            ...
            select array_agg(id order by cnt_3m asc, cnt_ano asc, id asc) into v_ordenados_h2
              from (
                  select cand.id,
                         (select count(*) from escala_semanal e
