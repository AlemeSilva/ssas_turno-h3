-- =====================================================================
-- array_agg(id order by cnt asc) nas escolhas de H3 e H2 não tinha
-- nenhum critério de desempate. Quando dois candidatos empatam na
-- contagem dos últimos 3 meses — frequente, já que o Caique sai de
-- contenção cedo todos os meses por causa do limite_h3_mensal=1 — o
-- desempate cai numa ordem interna do Postgres não especificada, que
-- na prática favorece sempre a mesma pessoa.
--
-- Achado no stress-test do preenchimento de 2027 (2026-09-10): correr
-- preencher_escala_anual() 3 vezes seguidas contra a equipa real
-- (Bruno/Caique limite 3, Caique limite 1, Kilson limite 3) deu SEMPRE
-- a mesma distribuição de H3 — Bruno 17, Caique 12 (no limite), Kilson
-- 23 — em vez de Bruno e Kilson (os dois sem limite efetivo) dividirem
-- as ~40 semanas restantes por igual.
--
-- Corrige com 2 níveis de desempate, ambos deterministicos:
--   1) (já existia) menor contagem nos últimos 3 meses — janela
--      recente, preserva o espírito original da rotação;
--   2) (novo) empate? menor contagem desde 1 de janeiro do próprio ano
--      a gerar — justiça acumulada dentro do ano, em vez de cair numa
--      ordem arbitrária do Postgres. Testado ao vivo contra 2027 antes
--      desta migração (função descartável, revertida): Bruno e Kilson
--      passam a 20/20, Caique mantém-se em 12 (o seu limite mensal não
--      muda com o desempate — continua a ser a trava, não o resultado
--      dela).
-- Um id como último recurso (3º nível) só entra nas primeiras semanas
-- do ano, quando tudo ainda está 0-0 — garante determinismo total sem
-- afetar o resultado prático.
--
-- Mesma correção aplicada à escolha de H2 (mesma falta de desempate,
-- mesma causa do desequilíbrio 8/15 no H4 residual entre Bruno/Caique
-- quando o Kilson fica de H3).
-- =====================================================================
