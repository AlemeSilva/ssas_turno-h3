-- =====================================================================
-- v_ficam_h1, v_ficam_h4 e v_elegiveis_h2 são calculados uma vez,
-- antes do loop semanal. Se nenhum OPERADOR ativo tiver
-- turno_fixo='H1' (ou 'H4'), ou nenhum OPERADOR_H3 ativo tiver
-- elegivel_h2=true, esses arrays ficam vazios — e a inserção
-- correspondente é simplesmente saltada, todas as semanas do ano,
-- sem nada assinalar isso como anómalo. A função regista sucesso na
-- mesma.
--
-- Ao contrário do mínimo de 3 no trio (Achado #1, um bloqueio — sem
-- isso não há H3/H2/H4 nenhum), um H1/H4-fixo/H2 vazio é um estado
-- válido — só deixa de ser silencioso. Uma única entrada de log por
-- execução continua a valer (mesma razão do Achado #2): o aviso entra
-- como uma terceira classificação, nunca uma segunda entrada.
-- =====================================================================
