...
// NOTA: page.clock.install() congela TODOS os timers JavaScript, incluindo
// os internos do Supabase. Por isso é SEMPRE chamado DEPOIS de loginComo()
// (que precisa de timers reais para gerir a sessão).
...
test.describe('Alerta sonoro/visual — janelas críticas 20h / 15h', () => {
  ...
  test('a partir das 20h de Sábado, o alerta e o botão de acionamento aparecem', async ({ page }) => {
    await loginComo(page, 'kilson')
    await page.clock.install({ time: new Date(`${SABADO_CICLO}T20:05:00`) })
    await page.getByRole('link', { name: 'Checklist Ativo' }).click()
  ...
  test('a barra superior mostra o próximo alerta e a contagem decrescente', async ({ page }) => {
    await loginComo(page, 'kilson')
    await page.clock.install({ time: new Date(`${SABADO_CICLO}T19:30:00`) })
    // Reload para que o AlertBar re-renderize com o relógio simulado
    await page.reload()
